...
// next load. This function is called directly by Telegram/Slack though (server-to-server, no
// browser involved), so it can't wait for that — it backfills the same two trackers itself,
// in-memory here and merged into what gets written back below, so it self-heals the first time
// either bot receives a message rather than requiring the person to open the app first.
