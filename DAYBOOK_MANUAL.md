...
*Covers the app as built through v24 (July 2026). Written for Craig as a complete reference — what's built, how it behaves, and how it's put together under the hood.*
...
- **Actions (new in v24)**: a flat, separate list (no hierarchy, no area tagging) for what kind of action a task is — Call, Meeting, Decide, Email, Follow-up, Errand, Research, Review out of the box. Same archive/"in use ×N"/permanent-delete pattern as Categories. This supplements the task's existing Type field (To-do/To-call/Follow-up) rather than replacing it — Type still drives the call list and auto-follow-up scheduling; Action is an extra, optional tag for filtering, bulk-editing, and reporting on top of that.
