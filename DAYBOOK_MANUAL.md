...
*Covers the app as built through v22 (July 2026). Written for Craig as a complete reference — what's built, how it behaves, and how it's put together under the hood.*
...
**List view (new in v22)**: a proper table, not just rows — Title, Type, Area, Project, Category, Priority, Status, Due as real columns, click any column header to sort by it (click again to reverse), Status editable inline per row, and the same bulk-select checkboxes as every other view. Shows whatever the current view's filters would show under "Everything" (open tasks, respecting search/area/priority/category). This is the view to reach for when you want to eyeball everything at once and clean house — scan for what's missing an area, batch-reprioritize a pile of P2s, or bulk-move a stack of tasks into a project.

**CSV export/import**: Export writes Title/Type/Area/Project/Priority/Status/Due/Created to a CSV named for the current view and date (or just the selected tasks, if any are selected). Import is a full bulk-loader with a downloadable template (including an inline instructions row listing every valid value), a hand-written CSV parser (handles quoted fields, embedded commas/newlines), per-field validation with warnings shown in a preview step before committing (bad priority/status/type values fall back to sensible defaults with a warning rather than failing the row), and name-based matching against your existing areas/projects/categories/people/vendors.
